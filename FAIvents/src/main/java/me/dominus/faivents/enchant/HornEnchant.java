...
public final class HornEnchant extends Enchantment {
    ...
    private HornEnchant(NamespacedKey key) {
        super(key);
    }

    public static HornEnchant get() {
        return instance;
    }
    ...
    public static void register(JavaPlugin plugin) {
        ...
        key = new NamespacedKey(plugin, "hero_horn");
        Enchantment existing = Enchantment.getByKey(key);
        if (existing instanceof HornEnchant) {
            instance = (HornEnchant) existing;
            return;
        }
        HornEnchant enchant = new HornEnchant(key);
        try {
